 ...
 *
 * Description: A Page Allocator/deallocator to support dynamic swap and
 *		demand paging.
 *
 * License:
...
static bool isMultiBootInfoSet;		// Is set to 0 (false) by default
static bool isMultiBootMmapAvail;	// Is set to 0 (false) by default
...
void page_map_init(multiboot_info_t *info , unsigned long lmagic)
{
		...
		kprintf("KERN_START_VM = [0x%x] KERN_END_VM = [0x%x] \n",KERN_START,KERN_END);
		kprintf("SETUP_END_VM = [0x%x] SETUP_END_VM = [0x%x] \n",SETUP_START,SETUP_END);
		...
		if(cmdline_start_pa)
		{
			kprintf("cmdline_start_pa = 0x%x cmdline_end_pa = 0x%x \n",cmdline_start_pa,cmdline_end_pa);
...
uint32_t phy_mmap(vm_offset_t start , vm_offset_t end , vm_offset_t *out_addr , bool page_alloc_needed , uint32_t alloc_size)
{
	...
	/*Exhausted the higher memory area*/
	if(phys_last_addr - phy_next_avail_addr == 0) goto try_low_mem;

	/*invalid input*/
	if(out_addr == NULL) goto error;

	retry:
		/* Advance ourselves to see where we are */
		kprintf("%s %d [in retry ]phy_next_avail_addr = [0x%x] alloc_size = [0x%x] \n",__FUNCTION__,__LINE__,phy_next_avail_addr,alloc_size);
		phy_next_avail_addr+=alloc_size;
		/* Is it usable ? */
		if (CHECK_FLAG (mbi.flags, 6))
		{
			bool isInMap = false;
					 ...
					 + mmap->size + sizeof (mmap->size)))
			{
				if (mmap->type == 0x1) //Take the usable region
				{
					...
					{
						kprintf("%s %d [in map ]phy_next_avail_addr = [0x%x] alloc_size = [0x%x] \n",__FUNCTION__,__LINE__,phy_next_avail_addr,alloc_size);
						isInMap = true;
						break;
					}
				}
			...
			if(!isInMap)
			{
				if(phys_last_addr - phy_next_avail_addr == 0)
				goto try_low_mem;
				phy_next_avail_addr=mmap->addr + mmap->len;
				goto  retry;
			}
		}
		...
		if ((phy_next_avail_addr > (mbi.mem_lower * 0x400)) && (phy_next_avail_addr - alloc_size  < 0x100000))
		{
			kprintf("%s %d [in BIOS ]phy_next_avail_addr = [0x%x] alloc_size = [0x%x] \n",__FUNCTION__,__LINE__,phy_next_avail_addr,alloc_size);
			phy_next_avail_addr = 0x100000;
			goto retry;
		}
		...
		if(page_alloc_needed)
		{
			...
			kprintf("%s %d [in page]phy_next_avail_addr = [0x%x] alloc_size = [0x%x] \n",__FUNCTION__,__LINE__,phy_next_avail_addr,alloc_size);
			needed = alloc_size;
	...
	try_low_mem:
		if(phys_last_addr - phy_next_avail_addr != 0)
		{
			kprintf("%s %d [in low mem ]phy_next_avail_addr = [0x%x] alloc_size = [0x%x] \n",__FUNCTION__,__LINE__,phy_next_avail_addr,alloc_size);
			phy_next_avail_addr = 0x1000;//start from 4kb
			phys_last_addr = 16 * 0x400 * 0x400;// last memory is 16mb
			goto retry;
		}
		else
		goto error;
	error:
		//allocation failure
		kprintf("%s %d [in error ]phy_next_avail_addr = [0x%x] alloc_size = [0x%x] \n",__FUNCTION__,__LINE__,phy_next_avail_addr,alloc_size);
		out_addr = NULL;
		ret = E_PHY_MEM_ALLOC_FAIL;
		return ret;
}

/* Used for local allocation functions. */
uint32_t alloc_phy_mem(vm_size_t size, vm_offset_t *out_addr)
{
